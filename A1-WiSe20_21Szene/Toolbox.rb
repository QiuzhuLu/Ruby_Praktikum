...
class Toolbox 
  ...
  :private
  def show_me()
    #TkRoot class represents the root widget. The root widget is at the top of the Ruby/Tk widget hierarchy
    # easy example:
    # require 'tk'
  ...
  def add_menu_leiste() 
    ...
    menubar.add :cascade, :menu => klassen_menu, :label => $CLASSMENU#cascade级联
  ...
  # Instanzen enthält
      ...
      if not @instanzen_menus.has_key?(klasse)
         ...
         $METHOD_CONFIG[ klasse_symbol ].each_pair { |method_name, method_config|
           menu.add(:command,
             ...
             :command => proc { 
               instanz = @instanzen[ @instanzen_listbox.curselection()[0]]#curselection当前选择
               if !(instanz.respond_to?(method_name))
                 ...
                 if(method_config.has_key?(:ergebnis) ) # von der Method Position, :ergebnis: Punkt
                   ergebnis = instanz.method(method_name).call()
                   Tk::messageBox(
                     :message => " #{instanz.class}'s #{method_name} :: #{ergebnis}  ",
                     :title => "Methodenergebnis #{method_name}"
                     )
                 elsif (method_config.has_key?(:args))# von der Method auf_position_setzen  :args => [[:x, :Integer] ,[:y,:Integer]]
                   # method_config ist ein element von $method_config, die in EinstellungenSzene ist.
   ...
   def bind_popup(menu)
    ...
    def instanz_hinzufuegen(instanz)
      @instanzen << instanz    #instanz wird in @instanzen hinzugefuert
      @instanzen_listbox.insert(:end, @instanzen.last().intern_to_s)#vermutilich ist @instazen,last() "instanz"(die letzte Elemente)
    end
