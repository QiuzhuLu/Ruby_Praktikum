...
class Toolbox 
  ...
  def show_me() 
     ...
     content = TkFrame.new(@root){
          height 300
          width 200}.grid(:row => 0, :column => 0, :sticky => "nswe") 
     add_instanzen_liste(content)
     TkGrid.columnconfigure( content, 0, :weight => 2 )
  ...
  def add_menu_leiste() 
    ...
    menubar.add :cascade, :menu => klassen_menu, :label => $CLASSMENU #Kaskade
   ...
   def add_instanzen_liste(parent)
     
     TkLabel.new(parent){text $INSTANCES }.grid(:row => 0, :column => 0, :sticky => "nwe") 

     @instanzen_listbox = TkListbox.new(parent) do
      width 30
      height 20
     end.grid(:row => 1, :column => 0, :sticky => "nesw")
     #Spalten und Zeilen.
     # If you want the widget to expand to fill up the entire cell, grid it with a sticky value of nsew (north, south, east, west), meaning it will stick to every side. This is shown
     ...
     scrollbar = Tk::Tile::Scrollbar.new(parent){
       orient 'vertical'}.grid( :row => 1, :column => 1,  :sticky => 'ns')
    ...
    @instanzen_listbox.bind '<ListboxSelect>', proc{
     ...
     # Verhindert, dass Selektionen in anderen Listen / Comboboxen die Selektion in der Instanzenliste verändert
     @instanzen_listbox.exportselection(false)
  end
             ...
             :command => proc { 
               ...
               if !(instanz.respond_to?(method_name))
                 ...
                 if(method_config.has_key?(:ergebnis) )
                   ...
                   Tk::messageBox(
                     :message => " #{instanz.intern_to_s}'s #{method_name} :: #{ergebnis}  ", 
